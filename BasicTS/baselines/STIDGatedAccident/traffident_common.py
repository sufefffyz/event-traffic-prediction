...
from .arch import STIDGatedAccident
...
def make_cfg(dataset_slug: str, num_nodes: int):
    ...
    model_param = {
        ...
        "residual_num_layer": 2,
        "gate_init_bias": -2.0,
    }

    cfg = EasyDict()
    cfg.DESCRIPTION = (
        "STID with a gated accident residual router on TraffiDent county subsets. "
        "The residual branch is masked to observed accident-active history windows."
    )
    cfg.GPU_NUM = 1
    ...
    cfg.WANDB.GROUP = "TraffiDent_2023Q1_STIDGatedAccident"
    cfg.WANDB.RUN_NAME = f"STIDGatedAccident_{data_name}_seed42"
    cfg.WANDB.TAGS = ["TraffiDent", data_name, "STIDGatedAccident", "basicts", "seed42"]
    ...
    cfg.MODEL = EasyDict()
    cfg.MODEL.NAME = STIDGatedAccident.__name__
    cfg.MODEL.ARCH = STIDGatedAccident
    ...
    cfg.TRAIN.CKPT_SAVE_DIR = os.path.join(
        "checkpoints",
        STIDGatedAccident.__name__,
        "_".join([data_name, str(NUM_EPOCHS), str(INPUT_LEN), str(OUTPUT_LEN), "gated_accident"]),
    )

...
class STIDGatedAccident(nn.Module):
    """STID with a sparse accident residual router.

    The base branch is the pure STID forecast. The accident branch predicts an
    additive residual, but the residual is multiplied by a learned gate and by
    the observed accident-active mask, giving the model a no-change option.
    """
    ...
    def __init__(self, **model_args):
        ...
        self.accident_dim = model_args.get("accident_dim", 32)
        ...
        self.gate_init_bias = model_args.get("gate_init_bias", -2.0)
        ...
        self.accident_emb = nn.Embedding(2, self.accident_dim)
        ...
        self.router_hidden_dim = self.base_hidden_dim + self.accident_dim
    ...
    def forward(
        ...
        **kwargs,
        ...
        accident_active = (
            history_data[..., self.accident_feature_index].sum(dim=1) > 0
        ).long()
        accident_emb = self.accident_emb(accident_active).transpose(1, 2).unsqueeze(-1)
        router_hidden = torch.cat([base_hidden, accident_emb], dim=1)
        router_hidden = self.router_encoder(router_hidden)

        gate = torch.sigmoid(self.gate_layer(router_hidden))
        active_mask = accident_active.unsqueeze(1).unsqueeze(-1).to(gate.dtype)

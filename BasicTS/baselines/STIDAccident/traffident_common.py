...
def make_cfg(dataset_slug: str, num_nodes: int):
    ...
    cfg = EasyDict()
    cfg.DESCRIPTION = (
        "STID plus binary accident embedding on TraffiDent county subset. Data is "
        "prepared from the official TraffiDent archive with official-style incident matching."
    )
    cfg.GPU_NUM = 1
    ...
    cfg.WANDB.GROUP = "TraffiDent_2023Q1_STIDAccident"
    cfg.WANDB.RUN_NAME = f"STIDAccident_{data_name}_seed42"
    cfg.WANDB.TAGS = ["TraffiDent", data_name, "STIDAccident", "basicts", "seed42"]
    ...
    cfg.MODEL = EasyDict()
    cfg.MODEL.NAME = STIDAccident.__name__
    cfg.MODEL.ARCH = STIDAccident

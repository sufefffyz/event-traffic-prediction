...
from .arch import STIDOracleFutureAccident
...
def make_cfg(dataset_slug: str, num_nodes: int):
    ...
    cfg = EasyDict()
    cfg.DESCRIPTION = (
        "Oracle future-accident STID upper-bound diagnostic on TraffiDent "
        "county subsets. The residual branch reads future accident covariates "
        "but never future target flow."
    )
    cfg.GPU_NUM = 1
    ...
    cfg.WANDB.GROUP = "TraffiDent_2023Q1_STIDOracleFutureAccident"
    cfg.WANDB.RUN_NAME = f"STIDOracleFutureAccident_{data_name}_seed42"
    cfg.WANDB.TAGS = [
        "TraffiDent",
        data_name,
        "STIDOracleFutureAccident",
        "oracle_future_event",
        "basicts",
        "seed42",
    ]
    ...
    cfg.MODEL = EasyDict()
    cfg.MODEL.NAME = STIDOracleFutureAccident.__name__
    cfg.MODEL.ARCH = STIDOracleFutureAccident
    ...
    cfg.TRAIN.CKPT_SAVE_DIR = os.path.join(
        "checkpoints",
        STIDOracleFutureAccident.__name__,
        "_".join(
            [
                data_name,
                str(NUM_EPOCHS),
